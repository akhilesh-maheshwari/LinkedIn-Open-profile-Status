...
try {
  ...
  if (!rawUrls.length)        throw new Error('At least one LinkedIn profile URL is required!');
  ...
  if (!validUrls.length) throw new Error('No valid LinkedIn profile URLs found! URLs must start with https://www.linkedin.com/in/');
  ...
  console.log('Profile count  :', rowCount);
    ...
    for (const result of batchStatusResults) {
      ...
      if (rowsPushed > 0) {
        ...
        try {
          ...
          console.log(`✅ Profiles delivered : ${totalRowsDelivered}`);
          console.log(`💳 Total charged      : $${totalCharged.toFixed(3)}`);
          console.log(`⏳ Remaining profiles : ${remainingLeads} (needs $${remainingCost} more)`);
          console.log(`👉 Add funds at apify.com/billing and re-run to get remaining profiles.`);
  ...
  console.log('Total Charged    : $', totalCharged.toFixed(3));
  console.log('Total Profiles   :', totalRowsDelivered);
  console.log('\nOutput Links:');
